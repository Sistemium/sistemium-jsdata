...
export default class Model {
  ...
  async fetchAll(query = {}, options) {
    ...
    const findNextPage = async done => {
      const opts = { ...options };
      try {
        const fetched = await this.store.findAll(name, { ...query, 'x-offset:': offset }, opts);
    ...
    await whilstAsync(() => offset && lastResultLength >= query.limit, findNextPage);
